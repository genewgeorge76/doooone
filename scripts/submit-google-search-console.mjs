 ...
 * ---------------------------------------------------------------
 * Auto-submits sitemap URLs to Google Search Console during Netlify
 * production builds for static sites (no npm lifecycle required).
 *
 * Required env:
 *   GSC_SERVICE_ACCOUNT_JSON  full service-account JSON (string)
 *
 * Optional env:
 *   GSC_SITE_URL            base site URL for sitemap discovery
 *   GSC_PROPERTY_SITE_URL   Search Console property identifier
 *                           (e.g. sc-domain:example.com)
 *   GSC_SITEMAP_URL         legacy single sitemap URL
 *   GSC_SITEMAP_URLS        comma/newline-separated sitemap URLs
 *   GSC_FORCE=1             run outside production context
 * ---------------------------------------------------------------
 */
